...
static date::time_zone const* find_time_zone(WEnvironment const& env)
{
    if (env.javaScript()) {
        if (string tz_name = env.timeZoneName(); ! tz_name.empty())
            if (auto tz = date::locate_zone(tz_name)) return tz;

        // Or this for old IE:
        // chrono::minutes tz_offset = env.timeZoneOffset();
    }

    if (auto tz = date::locate_zone(CONFIG.server_time_zone)) return tz;
    ...
    , session_(pool)
{
    ...
    if (any_of(abbrev.begin(), abbrev.end(),
...
void Application_controller::handle_internal_path(std::string internal_path)
{
    ...
    if (internal_path.empty()) { internal_path = "/"; }
    ...
    try {
        ...
        // /hw
            ...
            case User::Role::Student: add_tilde_user(); break;
            ...
            case User::Role::Grader: setInternalPath(Path::grade, true); break;

            case User::Role::Admin: setInternalPath(Path::admin, true); break;
            }
                                  ...
                                  Wt::Dbo::ptr<User> const& current)
{
    ...
    if (auto user = User::find_by_name(session_, user_name)) { return user; }
