...
Submission::Status Submission::status() const
{
    ...
    if (now <= assignment()->open_date()) { return Status::future; }
...
{
    if (is_loaded_) { return eval_status_; }
                                ...
                                const std::string& explanation)
{
    ...
    if (score == 0.0 && eval_item->type() == Eval_item::Type::Boolean) {
        ...
        if (frozen_score) submission->hold_grader_eval(sequence);
        else
...
void Submission::check_can_view(const dbo::ptr<User>& user) const
{
    ...
    if (! can_view(user)) throw Access_check_failed("That isn’t yours.");
}
...
{
    if (is_loaded_) { return grading_status_; }
...
void Submission::reload_cache() const
{
    ...
    eval_status_ =
            eval_status_given_counts_(self_eval_count, grader_eval_count);
    grading_status_ =
...
struct Partition_of_files {
    ...
    {
        if (file->is_automatically_deletable()) cheap.push_back(file);
        else
                     ...
                     dbo::ptr<Submission> const& sub2)
{
    ...
    if (partition.precious.empty()) {
        for (auto file : partition.cheap) { file.remove(); }
    } else {
...
void Submission::divorce()
{
    ...
    if (! old_user2) throw No_partner_to_separate{user1()->name()};
...
Self_eval_vec Submission::self_eval_vec() const
{
    ...
    for (const auto& self_eval : self_evals_) { insert(self_eval); }
