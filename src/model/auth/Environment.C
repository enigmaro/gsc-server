...
{
    if (auto app = Wt::WApplication::instance()) return &app->environment();
    else
        return nullptr;
}
...
bool App_environment::invalid() const
{
...
bool Req_environment::invalid() const
{
    return false;
}
...
optional<string> param_whoami(Environment const& env)
{
    ...
    if (auto whoami = env.get_parameter("whoami")) return {*whoami};
    else
        return nullopt;
}
...
optional<string> env_remote_user(Environment const& env)
{
    ...
    if (auto whoami = param_whoami(env)) return whoami;
    else
