                        ...
                        std::ostream& message)
{
    ...
    auto my_submission = Submission::find_by_assignment_and_user(
            session, assignment, requestor);

    if (auto user2 = my_submission->user2()) {
        if (user2 == requestee) {
            message << s::user{requestee, "User"}
                    << " is already your partner for " << s::hw{assignment}
                    << ".";
        } else {
    ...
    if (other_submission->user2() ||
        ! other_submission->can_submit(requestee)) {
                                      ...
                                      const dbo::ptr<Assignment>& assignment)
{
    ...
    auto incoming = Partner_request::find_by_requestee_and_assignment(
            session, user, assignment);
    for (auto each : incoming) { each.remove(); }
}
...
{
    // Alphabetical order
    if (user1->name() > user2->name()) std::swap(user1, user2);
