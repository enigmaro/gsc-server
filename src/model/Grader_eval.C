...
{
    switch (status()) {
    case Status::ready: return Abstract_evaluation::plain_score_string();
    case Status::held_back: return "[held back]";
...
Score_owner Grader_eval::score_owner(Viewing_context const& cxt) const
{
    ...
    if (can_see_score_(cxt)) {
        ...
        if (! grader()->can_grade()) owner = "Auto";
...
void Grader_eval::check_can_view(dbo::ptr<User> const& user) const
{
    ...
    if (! self_eval()->submission()->can_view(user)) throw Resource_not_found();
