...
{
    if (type == Eval_item::Type::Boolean) return 0;
    else
        return 1;
}
                                      ...
                                      const dbo::ptr<Submission>& submission)
{
    ...
    auto result = session.add(
            unique_ptr<Self_eval>{new Self_eval{eval_item, submission}});
...
Self_eval::find_with_grade_status(Grader_eval::Status status, dbo::Session& dbo)
{
    return dbo
                                      ...
                                      Viewing_context const& cxt)
{
    ...
    case User::Role::Student: return "Other student";
    case User::Role::Grader: return "Student";
    case User::Role::Admin: return submission.owner_string();
    }
}
