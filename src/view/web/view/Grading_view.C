    ...
    , model_(self_eval)
{
    ...
    auto grading_widget =
            Abstract_grading_widget::create(grader_eval, session());

    std::string status;
    switch (grader_eval->status()) {
    case Grader_eval::Status::editing: status += "unsaved"; break;
    case Grader_eval::Status::held_back: status += "held"; break;
    ...
    widget->bindNew<Explanation_view_widget>(
            "self_explanation", self_eval->explanation(), file_viewer(),
            "student-highlight");
