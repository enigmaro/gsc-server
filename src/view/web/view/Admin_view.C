...
void Submission_chooser::go()
{
    ...
    if (current_index < (int) submissions_.size()) {
        ...
        case Submission::Status::extended: Navigate::to(submission->url());
...
{
    if (auto user = editor_->selected_user()) Navigate::to(user->hw_url());
}
    ...
    , combo_(dynamic_cast<WComboBox*>(implementation()))
{
    ...
    for (auto const& each : Enum<Role>::info) combo_->addItem(each.name);
...
{
    if (auto user = editor_->selected_user()) chooser_->setRole(user->role());
    else
...
void SU_widget::go()
{
    if (auto user = editor_->selected_user()) session_.become_user(user);
}
    ...
    : session_(session)
{
    ...
    auto ja = table->elementAt(row, 0)->addNew<Accelerator_text>(
            "Jump to su&bmission");
    ja->set_target(
            table->elementAt(row, 1)->addNew<Submission_chooser>(session_));

    auto js = table->elementAt(++row, 0)->addNew<Accelerator_text>(
            "Jump to &student:");
    js->set_target(table->elementAt(row, 1)->addNew<Student_chooser>(session_));

    auto cr = table->elementAt(++row, 0)->addNew<Accelerator_text>(
            "Change &role:");
    cr->set_target(table->elementAt(row, 1)->addNew<Role_updater>(session_));

    auto su = table->elementAt(++row, 0)->addNew<Accelerator_text>(
            "S&witch users:");
    su->set_target(table->elementAt(row, 1)->addNew<SU_widget>(session_));

    auto nu =
            table->elementAt(++row, 0)->addNew<Accelerator_text>("&New user:");
    ...
    auto gs = table->elementAt(++row, 1)->addNew<Accelerator_button>(
            "Grading s&tats");
    gs->clicked().connect(Navigate("/grading_stats"));

    auto play_game = table->elementAt(++row, 1)->addNew<Accelerator_button>(
            "&Play game");
