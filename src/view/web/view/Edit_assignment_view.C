...
Edit_eval_item::Edit_eval_item(const dbo::ptr<Eval_item>& eval_item,
    ...
    , in_edit_mode_(false)
{
    if (eval_item->prompt().empty()) edit_mode_();
    else
        view_mode_();
}
...
void Edit_eval_item::edit_mode_()
{
    ...
    value_ = templ->bindNew<WLineEdit>("value",
                                       eval_item_->relative_value_str());
...
{
    auto widget =
            container_->addNew<Edit_eval_item>(eval_item, *this, session_);
