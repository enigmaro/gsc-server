    ...
    : Submissions_view::Row_view(model, session, row)
{
    ...
    due_date_->changed().connect(
            this, &Admin_submissions_view_row::due_date_changed_);
...
void Admin_submissions_view_row::update()
{
    ...
    if (model_.submission->extended()) {
        ...
        auto date = model_.submission->assignment()
                            ->due_date()
                            .toLocalTime()
                            .toString();
                                      ...
                                      WTableRow* row)
{
    if (session.user()->can_admin()) {
        auto result =
                make_unique<Admin_submissions_view_row>(model, session, row);
        result->update();
        return move(result);
    } else {
        auto result =
