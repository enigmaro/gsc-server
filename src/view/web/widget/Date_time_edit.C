...
Wt::WValidator::State Date_time_edit::validate()
{
    ...
    if (date_edit_->text() == "" && time_edit_->text() == "") {
        if (mandatory_) return VS::InvalidEmpty;
        else
            return VS::Valid;
    }

    if (date_edit_->validate() != VS::Valid) return VS::Invalid;

    if (time_edit_->validate() != VS::Valid) return VS::Invalid;

    auto value = date_time();

    if (bottom_.isValid() && value < bottom_) return VS::Invalid;
    if (top_.isValid() && value > top_) return VS::Invalid;
