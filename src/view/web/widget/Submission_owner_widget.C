...
void Submission_owner_widget::reload_()
{
    ...
    case User::Role::Admin: update_admin_(); break;

    case User::Role::Grader: update_grader_(); break;

    case User::Role::Student: update_student_(); break;
    }
}
    ...
    if (submission()->user2() && (status == Submission::Status::open ||
        ...
        edit->enterPressed().connect([=]() {
            html_try<Notification>([=] {
                ...
                if (auto user2 = User::find_by_name(session(),
                                                    edit->text().toUTF8())) {
                    auto joined = Partner_request::confirm(
                            session(), submission()->user1(), user2,
...
void Submission_owner_widget::update_grader_()
{
    ...
    if (submission()->user2()) {
        message << " and <strong>" << submission()->user2()->name()
                << "</strong>";
    }
    transaction.commit();
...
void Submission_owner_widget::update_student_()
{
    ...
    if (submission()->user2()) {
        ...
        std::ostringstream message;
        message << "Partnered with <strong>" << other_user->name()
                << "</strong>";
        impl_->addNew<Wt::WText>(message.str());
        return;
    }
