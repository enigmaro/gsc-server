    ...
    , request_(request)
{
    ...
    message << "Your partner request to <strong>"
            << request_->requestee()->name() << "</strong>";
...
void Partner_requestor_widget::submit_()
{
    ...
    try {
        ...
        dbo::Transaction transaction(context_.session());
        auto user2 =
                User::find_by_name(context_.session(), edit_->text().toUTF8());
        if (! user2) {
        ...
        auto request = Partner_request::create(context_.session(),
                                               context_.principal(), user2,
...
void Partner_notification_widget::reload_()
{
    ...
    if (submission()) {
        ...
        auto incoming =
                Partner_request::find_by_requestee(session(), principal());
        for (const auto& each : incoming)
            if (each->is_active(session()))
                impl_->addNew<Partner_confirmer_widget>(context(), each, true);

        auto outgoing =
                Partner_request::find_by_requestor(session(), principal());
