...
static Credentials parse_authorization(const std::string& header_value)
{
    ...
    if (header_value.empty()) return Credentials{"", ""};
...
void Request_handler::check_password_strength(Credentials const& cred)
{
    ...
    Http_error{403} << "Please try a stronger password ("
                    << psv_result.message() << ")";
}
#endif  // GSC_AUTH_PASSWORD
...
dbo::ptr<User> Request_handler::authenticate_by_cookie_() const
{
    ...
    // Intercept logouts
    if (std::regex_match(path_info_, paths::Whoami::re) &&
        method_ == "DELETE") {
        auto hash =
                session_.auth().tokenHashFunction()->compute(*in_cookie, "");
    ...
    if (auth_result.state() != Wt::Auth::AuthTokenState::Valid) return {};
...
dbo::ptr<User> Request_handler::authenticate_by_password_() const
{
    ...
    auto verify_result =
            service.verifyPassword(auth_user, credentials.password);
    ...
    case PasswordResult::PasswordValid: create_cookie_(auth_user); return user;
    }
...
{
    if (auto auth_info = session_.find_from_environment<Auth_info>(
